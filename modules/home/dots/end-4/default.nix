...
let
  ...
  oneUIIconsPath = "${pkgs.telperion.illogical-impulse-oneui4-icons}/share/icons";

  cfg = config.telperion.dots.end-4;
in
{
  options.telperion.dots.end-4 = {
    enable = lib.mkEnableOption "Use Illogical End-4 dotfiles";
  };
  ...
  config = mkIf (cfg.enable && pkgs.stdenv.hostPlatform.isLinux) {
    ...
    gtk = {
      ...
      iconTheme = {
        ...
        package = mkDefault pkgs.telperion.illogical-impulse-oneui4-icons;
      };
    };
