...
let
  ...
  cfg = config.telperion.programs.terminal.tools.atuin;

in
{
  options.telperion.programs.terminal.tools.atuin = {
  ...
  config = mkIf cfg.enable {
    launchd.agents.atuin-daemon.config = lib.mkIf pkgs.stdenv.hostPlatform.isDarwin {
      StandardErrorPath = osConfig.telperion.programs.terminal.tools.atuin.logPaths.stderr;
