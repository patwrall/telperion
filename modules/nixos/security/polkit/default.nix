...
let
  cfg = config.telperion.security.polkit;
in
{
  options.telperion.security.polkit = {
  ...
  config = lib.mkIf cfg.enable {
    environment.systemPackages =
      with pkgs;
      lib.optionals (!config.telperion.programs.graphical.wms.hyprland.enable) [
    ...
    systemd = {
      user.services = {
        polkit-kde-authentication-agent-1 =
          lib.mkIf (!config.telperion.programs.graphical.wms.hyprland.enable)
            {
